...
object SimpleSentimentAnalysis {
  ...
  val positive = Source.fromInputStream(getClass.getResourceAsStream("/" + POSITIVE_FILE))("UTF-8").mkString
  ...
  val negative = Source.fromInputStream(getClass.getResourceAsStream("/" + NEGATIVE_FILE))("UTF-8").mkString
